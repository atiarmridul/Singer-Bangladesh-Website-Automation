...
test.describe("AI generated tests", () => {
  ...
  test("@ai @catalog @sanity @product SANITY_013 Product - should show product images", async ({ page }) => {
    ...
    await expect(
      page.locator("main img[src*='product'], main img[alt], main [class*='gallery'] img").first()
    ).toBeVisible();
  });
});
