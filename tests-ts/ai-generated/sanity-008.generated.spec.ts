...
test.describe("AI generated tests", () => {
  ...
  test("@ai @catalog @sanity @auth SANITY_008 Auth - should open login modal from homepage", async ({ page }) => {
    await page.goto("/", { waitUntil: "domcontentloaded" });
    await dismissBlockingModals(page);
    await clickOrNavigate(page, page.locator("button:visible:has-text('Log In')"));
    await expect(page.getByText("Welcome to Singer").first()).toContainText("Welcome to Singer");
  });
});
