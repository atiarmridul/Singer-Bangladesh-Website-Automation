...
2. `environments/*.env` stage/prod profiles
...
- **JSON Definitions:** Define test cases in structured JSON under `ai/definitions/`. This makes test logic portable and easy for AI to parse.
- **Spec Generation:** Use a script such as `ai/generate-test.ts` to turn JSON definitions into standard Playwright `.spec.ts` files.
